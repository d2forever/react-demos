...
const RIKKEI_POST =
  'https://forum.rikkei.vn/index.php?threads/react-basic-handle-user-input-with-state-and-events-part-05.23';
  ...
  onSearchSubmitHandler = async (searchTerm) => {
    try {
      ...
      if (response.data.results.length === 0 && searchTerm !== '') {
        throw new Error(
          `Can't found image with keyword '${searchTerm}'. Please try again!`
        );
      }
